...
public class HeapPriorityQueue<V> {
	...
	* */
	...
	private void moveUp(int i) {
		...
		while (i > 0) {
			if (ep.priority >= heap[(i - 1) / 2].priority) break; // Lo fa in caso la priorità dell'elemento è più grande di quella del padre
			heap[i] = heap[(i - 1) / 2];
	...
	public void printHeap() {
		for (int i = 0; i < indiceUltimo; i++) {
			position.toString();
			System.out.println("Elemento: " + heap[i].element + ", Priorità: " + heap[i].priority + " Indice nell'hash: " +
