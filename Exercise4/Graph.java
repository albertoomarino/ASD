...
public class Graph<G> {
	...
	* It creates a new Arch wich will be appended to the source's adjiacency list
	...
	*/
	...
	public void add_arch(G source, G destination, double i)
	...
	*/
	...
	public boolean has_already_Arch(G source, G destination)
		throws GraphException {
		if (source == null || destination == null) {
	...
	*/
	...
	public void remove_arch(G src, G dest) throws GraphException {
		if (src == null || dest == null) throw new GraphException("Error: a null arch cannot be removed");
	
		if (!map.containsKey(src) || !map.containsKey(dest)) throw new GraphException("Error: cannot remove an arch with a null boundary");
	...
	*/
	...
	public List<G> get_vertexes() {
		...
		for (G k : map.keySet()) v_list.add(k);
		return v_list;
	}
	...
	*/
	...
	public List<G> adj_list_of(G vertex) throws GraphException {
		...
		for (int i = 0; i < map.get(vertex).size(); i++) adj_list.add(map.get(vertex).get(i).getEnd());
		return adj_list;
	}
	...
	*/
	...
	public double get_w_btw(G v1, G v2) throws GraphException {
		if (v1 == null || v2 == null) throw new GraphException("Error: get_w_btw: Parameters cannot be null");
	
		if (has_already_Arch(v1, v2)) {
			for (int i = 0; i < map.get(v1).size(); i++) if (
				map.get(v1).get(i).getEnd() == v2
			) return map.get(v1).get(i).getWeight();
		} else throw new GraphException("Error: get_w_btw: arch (" + v1 + ", " + v2 + ") doesn't exist");
		return -1;
	}
	...
	*/
	...
	public Graph<String> minPath(Graph<String> graph, String start) {
		HeapPriorityQueue<String> minQueue = new HeapPriorityQueue<String>(graph.get_vertexes().size());
