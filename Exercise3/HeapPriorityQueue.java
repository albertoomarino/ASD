...
public class HeapPriorityQueue<V> {
	...
	*/
	...
	/**
	* moveUp function, wich in the event of an element out of place makes it rise up to the right place
	...
	*/
	...
	private void moveUp(int i) {
		...
		while (i > 0) {
			if (ep.priority >= heap[(i - 1) / 2].priority) break; // Lo fa in caso la priorità dell'elemnto è più grande di quella del padre
			heap[i] = heap[(i - 1) / 2];
	...
	*/
	...
	public void rialloca() {
		...
		for (int i = 0; i < heap.length; i++) nuovo[i] = heap[i];
		heap = nuovo;
	}
	...
	*/
	...
	public void printHash() {
		for (int i = 0; i < indiceUltimo; i++) System.out.println(
		"Hash: " +
		position.get(heap[i].element) +
		"Elemento: " +
